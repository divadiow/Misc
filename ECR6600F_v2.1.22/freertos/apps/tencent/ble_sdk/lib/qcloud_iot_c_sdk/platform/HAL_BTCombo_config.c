...
static void bt_combo_report_wificonn_success()
{
    ...
    read_len = qiot_flash_GetString("StaSSID", ssid, WIFI_SSID_MAX_LEN-1);
    if (read_len == 0) {
        PUSH_LOG("Get SSID failed!!\r\n");
    } else {
       PUSH_LOG("Get SSID success!!\r\n");
    }
    //ble_event_report_wifi_connect(BLE_WIFI_MODE_STA, BLE_WIFI_STATE_CONNECT, uint8_t ssid_len, const char *ssid);
    ble_event_report_wifi_connect(BLE_WIFI_MODE_STA, BLE_WIFI_STATE_CONNECT, (uint8_t)strlen((const char *)ssid), ssid);
}
