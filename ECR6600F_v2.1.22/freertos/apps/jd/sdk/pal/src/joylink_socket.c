...
int32_t jl_platform_gethostbyname(char *hostname, char *ip_buff, uint32_t buff_len)
{
    ...
    int ret;
    char *family, *socktype = NULL, *protocol = NULL;
    const char *ip;
    void *sin_addr = NULL;
    ...
    *********************************/
    ...
    if (ret != 0) {
        jl_platform_printf("getaddrinfo() failed, ret = %d(%s)", ret, strerror(ret));
        return -1;
    }
    ...
    freeaddrinfo(res);
    return 0;
#elif defined(__LINUX_PAL__)  || defined(CONFIG_JD_FREERTOS_PAL)
...
{
#if defined (__LINUX_PAL__) || defined (CONFIG_JD_FREERTOS_PAL)
    int32_t in_domain = 2, in_type = 1, in_protocol = 0;
    if (domain == JL_SOCK_PROTO_DOMAIN_AF_INET) {
        in_domain = AF_INET;
    }
    else if (domain == JL_SOCK_PROTO_DOMAIN_AF_INET6) {
...
{
#if defined (__LINUX_PAL__) || defined (__ESP_32_PAL__) || defined (CONFIG_JD_FREERTOS_PAL)
    int32_t ret;
    fd_set fds;
    struct timeval tv;
