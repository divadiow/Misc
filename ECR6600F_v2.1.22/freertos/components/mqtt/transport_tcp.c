...
static int tcp_connect(transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    if (tcp->sock < 0) {
        os_printf(LM_APP, LL_ERR, "TCP:Error create socket\n");
        return -1;
    }
    ...
    os_printf(LM_APP, LL_ERR, "TCP:[sock=%d],connecting to server IP:%s,Port:%d...\r\n",
...
static int tcp_close(transport_handle_t t)
{
    transport_tcp_t *tcp = transport_get_context_data(t);
    int ret = -1;
    os_printf(LM_APP, LL_ERR, "TCP:In tcp_close(), socket: %d\r\n", tcp->sock);
    if (tcp->sock >= 0) {
        ret = close(tcp->sock);
        tcp->sock = -1;
    }
