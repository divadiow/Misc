...
int httpd_send(httpd_req_t *r, const char *buf, size_t buf_len)
{
    ...
    if (ret < 0) {
        os_printf(LM_APP, LL_DBG, LOG_FMT("error in send_fn"));
        return ret;
    }
    return ret;
}

static int httpd_send_all(httpd_req_t *r, const char *buf, size_t buf_len)
{
    struct httpd_req_aux *ra = r->aux;
    int ret;
    ...
    while (buf_len > 0) {
        ...
        os_printf(LM_APP, LL_DBG, LOG_FMT("sent = %d"), ret);
        buf     += ret;
        buf_len -= ret;
    }
    return OS_SUCCESS;
...
{
    os_printf(LM_APP, LL_DBG, LOG_FMT("requested length = %d"), buf_len);
    ...
    if (ra->sd->pending_len > 0) {
        os_printf(LM_APP, LL_DBG, LOG_FMT("pending length = %d"), ra->sd->pending_len);
    ...
    if (ret < 0) {
        os_printf(LM_APP, LL_DBG, LOG_FMT("error in recv_fn"));
    ...
    os_printf(LM_APP, LL_DBG, LOG_FMT("received length = %d"), ret + pending_len);
...
size_t httpd_unrecv(struct httpd_req *r, const char *buf, size_t buf_len)
{
    ...
    os_printf(LM_APP, LL_DBG, LOG_FMT("length = %d"), ra->sd->pending_len);
...
int httpd_resp_set_hdr(httpd_req_t *r, const char *field, const char *value)
{
    ...
    os_printf(LM_APP, LL_DBG, LOG_FMT("new header = %s: %s"), field, value);
    return OS_SUCCESS;
}
...
int httpd_resp_send(httpd_req_t *r, const char *buf, ssize_t buf_len)
{
    ...
    if (httpd_send_all(r, ra->scratch, strlen(ra->scratch)) != OS_SUCCESS) {
        return ERR_HTTPD_RESP_SEND;
    }

    /* Sending additional headers based on set_header */
    for (unsigned i = 0; i < ra->resp_hdrs_count; i++) {
        /* Send header field */
        if (httpd_send_all(r, ra->resp_hdrs[i].field, strlen(ra->resp_hdrs[i].field)) != OS_SUCCESS) {
            return ERR_HTTPD_RESP_SEND;
        }
        /* Send ': ' */
        if (httpd_send_all(r, colon_separator, strlen(colon_separator)) != OS_SUCCESS) {
            return ERR_HTTPD_RESP_SEND;
        }
        /* Send header value */
        if (httpd_send_all(r, ra->resp_hdrs[i].value, strlen(ra->resp_hdrs[i].value)) != OS_SUCCESS) {
            return ERR_HTTPD_RESP_SEND;
        }
        /* Send CR + LF */
        if (httpd_send_all(r, cr_lf_seperator, strlen(cr_lf_seperator)) != OS_SUCCESS) {
            return ERR_HTTPD_RESP_SEND;
        }
    }
    ...
    /* Sending content */
    if (buf && buf_len) {
        if (httpd_send_all(r, buf, buf_len) != OS_SUCCESS) {
            return ERR_HTTPD_RESP_SEND;
        }
    }
    return OS_SUCCESS;
...
int httpd_resp_send_chunk(httpd_req_t *r, const char *buf, ssize_t buf_len)
{
    ...
    if (httpd_send_all(r, len_str, strlen(len_str)) != OS_SUCCESS) {
        return ERR_HTTPD_RESP_SEND;
    }

    if (buf) {
        if (httpd_send_all(r, buf, (size_t) buf_len) != OS_SUCCESS) {
            return ERR_HTTPD_RESP_SEND;
        }
    }
...
int httpd_resp_send_err(httpd_req_t *req, httpd_err_code_t error, const char *usr_msg)
{
    int ret;
    const char *msg;
    const char *status;
    ...
    os_printf(LM_APP, LL_WARN, LOG_FMT("%s - %s"), status, msg);
    ...
    if (setsockopt(ra->sd->fd, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(nodelay)) < 0) {
        ...
        os_printf(LM_APP, LL_WARN, LOG_FMT("error calling setsockopt : %d"), errno);
        nodelay = 0;
    }
#endif
    ...
    if (nodelay == 1) {
        ...
        if (setsockopt(ra->sd->fd, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(nodelay)) < 0) {
            ...
            os_printf(LM_APP, LL_ERR, LOG_FMT("error calling setsockopt : %d"), errno);
...
int httpd_req_handle_err(httpd_req_t *req, httpd_err_code_t error)
{
    ...
    if (hd->err_handler_fns[error]) {
        ...
        ret = (error == HTTPD_500_INTERNAL_SERVER_ERROR ? OS_FAIL : ret);
    } else {
...
int httpd_req_recv(httpd_req_t *r, char *buf, size_t buf_len)
{
    ...
    if (!httpd_valid_req(r)) {
        os_printf(LM_APP, LL_WARN, LOG_FMT("invalid request"));
        return HTTPD_SOCK_ERR_INVALID;
    }

    struct httpd_req_aux *ra = r->aux;
    os_printf(LM_APP, LL_DBG, LOG_FMT("remaining length = %d"), ra->remaining_len);
    ...
    os_printf(LM_APP, LL_DBG, LOG_FMT("received length = %d"), ret);
    return ret;
}
...
{
    int errval;
    os_printf(LM_APP, LL_WARN, LOG_FMT("error in %s : %d"), ctx, errno);
