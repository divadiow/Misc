...
struct wpa_driver_associate_params {
	 ...
	 */
	//int pbss;
	const u8 *bss_rate_ie;
	const u8 *bss_ext_rate_ie;
	const u8 *bss_ht_cap_ie;
};
...
struct wpa_driver_ops {
    ...
    /**
	 * wpa_states_inactive - Indicate inactive state (wpa_supplicant disabled)
	 * @priv: Private driver interface data
	 * Returns: 0 on success, -1 on failure
	 */
    int (*wpa_states_inactive)(void *priv);
};
...
extern const struct wpa_driver_ops wpa_driver_atheros_ops;
	...
	extern const struct wpa_driver_ops wpa_driver_eswin_ops;
#endif /* CONFIG_DRIVER_FREERTOS */
