...
int hostapd_get_hw_features(struct hostapd_iface *iface)
{
    ...
    if (!modes->he_capab[IEEE80211_MODE_AP].he_supported)
    {
			  ...
			  struct hostapd_hw_modes *mode)
{
	...
	if (!iface->current_rates) {
		wpa_printf_error(MSG_ERROR, "Failed to allocate memory for rate "
			   "table.");
		return -1;
	}
	...
	if ((iface->num_rates == 0 || num_basic_rates == 0) &&
		...
		wpa_printf_error(MSG_ERROR, "No rates remaining in supported/basic "
			   "rate sets (%d,%d).",
...
static void ieee80211n_check_scan(struct hostapd_iface *iface)
{
	...
	if (!res) {
		...
		hostapd_set_oper_chwidth(iface->conf, VHT_CHANWIDTH_USE_HT);
		res = 1;
...
static int ieee80211n_supported_ht_capab(struct hostapd_iface *iface)
{
	...
	if ((conf & HT_CAP_INFO_LDPC_CODING_CAP) &&
		...
		wpa_printf_error(MSG_ERROR, "Driver does not support configured "
			   "HT capability [LDPC]");
		return 0;
	}
...
static int ieee80211ac_cap_check(u32 hw, u32 conf, u32 cap, const char *name)
{
	...
	if ((hw & req_cap) != req_cap) {
		wpa_printf_error(MSG_ERROR, "Driver does not support configured VHT capability [%s]",
			   name);
		return 0;
	}
	return 1;
				     ...
				     const char *name)
{
	...
	if (conf_val > hw_max) {
		wpa_printf_error(MSG_ERROR, "Configured VHT capability [%s] exceeds max value supported by the driver (%d > %d)",
...
int hostapd_acs_completed(struct hostapd_iface *iface, int err)
{
	...
	case HOSTAPD_CHAN_VALID:
		...
		break;
	case HOSTAPD_CHAN_ACS:
		wpa_printf_error(MSG_ERROR, "ACS error - reported complete, but no result available");
		wpa_msg(iface->bss[0]->msg_ctx, MSG_INFO, ACS_EVENT_FAILED);
		hostapd_notify_bad_chans(iface);
		goto out;
	case HOSTAPD_CHAN_INVALID:
	default:
		wpa_printf_error(MSG_ERROR, "ACS picked unusable channels");
		wpa_msg(iface->bss[0]->msg_ctx, MSG_INFO, ACS_EVENT_FAILED);
		hostapd_notify_bad_chans(iface);
		goto out;
	}
