...
void napt_send_tcp_rst(struct pbuf *p, const struct ip_hdr *iphdr, struct tcp_hdr *tcphdr)
{
    ...
    memcpy(&dst, &iphdr->dest, sizeof(ip_addr_t));
    memcpy(&src, &iphdr->src, sizeof(ip_addr_t));
    tcp_rst(NULL, lwip_ntohl(tcphdr->ackno), lwip_ntohl(tcphdr->seqno) + data_len, &dst,
       &src, lwip_ntohs(tcphdr->dest), lwip_ntohs(tcphdr->src));
...
err_t ip_napt_forward(struct pbuf *p, struct ip_hdr *iphdr, struct netif *inp, struct netif *outp)
{
    ...
    if (IPH_PROTO(iphdr) == IP_PROTO_ICMP) {
        ...
        if (iecho->type == ICMP_ECHO) {
            ...
            ip_napt_modify_addr(iphdr, (ip4_addr_p_t *)&iphdr->src, outp->ip_addr.u_addr.ip4.addr);
        }
        return ERR_OK;
    }
#endif
    ...
    if (IPH_PROTO(iphdr) == IP_PROTO_TCP) {
        ...
        PP_NTOHS(tcphdr->src) >= 1024*/) {
            ...
            if (!t || t->dest != iphdr->dest.addr || t->dport != tcphdr->dest) {
                os_printf(LM_APP, LL_DBG, "uplink not find napt table: from %d.%d.%d.%d : %d\n",
                        (iphdr->src.addr) & 0xff, (iphdr->src.addr >> 8) & 0xff, (iphdr->src.addr >> 16) & 0xff, 
        ...
        ip_napt_modify_addr_tcp(tcphdr, (ip4_addr_p_t *)&iphdr->src, outp->ip_addr.u_addr.ip4.addr);
        ip_napt_modify_addr(iphdr, (ip4_addr_p_t *)&iphdr->src, outp->ip_addr.u_addr.ip4.addr);
    ...
    if (IPH_PROTO(iphdr) == IP_PROTO_UDP) {
        ...
        ip_napt_modify_addr_udp(udphdr, (ip4_addr_p_t *)&iphdr->src, outp->ip_addr.u_addr.ip4.addr);
        ip_napt_modify_addr(iphdr, (ip4_addr_p_t *)&iphdr->src, outp->ip_addr.u_addr.ip4.addr);
