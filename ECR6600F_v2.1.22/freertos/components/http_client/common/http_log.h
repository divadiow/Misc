...
#else
    #define HTTP_LOG_D(fmt, ...)    { os_printf(LM_APP, LL_INFO, fmt, ##__VA_ARGS__); os_printf(LM_APP, LL_INFO, "\n");}
#endif

#if LOG_LEVEL < INFO_LEVEL
    #define HTTP_LOG_I(fmt, ...)
#else
    #define HTTP_LOG_I(fmt, ...)    { os_printf(LM_APP, LL_INFO, fmt, ##__VA_ARGS__); os_printf(LM_APP, LL_INFO, "\n");}
#endif

#if LOG_LEVEL < WARN_LEVEL
    #define HTTP_LOG_W(fmt, ...)
#else
    #define HTTP_LOG_W(fmt, ...)    { os_printf(LM_APP, LL_INFO, fmt, ##__VA_ARGS__); os_printf(LM_APP, LL_INFO, "\n");}
#endif

#if LOG_LEVEL < ERR_LEVEL
    #define HTTP_LOG_E(fmt, ...)
#else
    #define HTTP_LOG_E(fmt, ...)    { os_printf(LM_APP, LL_INFO, fmt, ##__VA_ARGS__); os_printf(LM_APP, LL_INFO, "\n");}
#endif

#if LOG_LEVEL < BASE_LEVEL
    #define HTTP_LOG(fmt, ...)
#else
    #define HTTP_LOG(fmt, ...)      { os_printf(LM_APP, LL_INFO, fmt, ##__VA_ARGS__); os_printf(LM_APP, LL_INFO, "\n");}
#endif
