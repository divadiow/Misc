...
{
    if (0 != *flags)
        HTTP_LOG_E("%s:%d %s()... server_certificate_verify failed returned 0x%04lx\n", __FILE__, __LINE__, __FUNCTION__, *flags);
    return *flags;
}
#endif
...
static int nettype_tls_init(network_t* n, nettype_tls_params_t* nettype_tls_params)
{
    ...
    //mbedtls_platform_set_calloc_free(platform_memory_calloc_mbedtls, platform_memory_free_mbedtls);
    ...
    if ((rc = mbedtls_ctr_drbg_seed(&(nettype_tls_params->ctr_drbg), mbedtls_entropy_func,
        ...
        HTTP_LOG_E("mbedtls_ctr_drbg_seed failed returned 0x%04x", (rc < 0 )? -rc : rc);
        RETURN_ERROR(rc);
    }

    if ((rc = mbedtls_ssl_config_defaults(&(nettype_tls_params->ssl_conf), MBEDTLS_SSL_IS_CLIENT,
                                           MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
        HTTP_LOG_E("mbedtls_ssl_config_defaults failed returned 0x%04x", (rc < 0 )? -rc : rc);
        RETURN_ERROR(rc);
    }
    ...
    if (NULL != n->ca_crt) {
        ...
        if (0 != (rc = (mbedtls_x509_crt_parse(&(nettype_tls_params->ca_cert), (unsigned char *)n->ca_crt,
                                          (n->ca_crt_len + 1))))) {
            HTTP_LOG_E("%s:%d %s()... parse ca crt failed returned 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
            RETURN_ERROR(rc);
        }
        ...
        if ((rc = mbedtls_ssl_conf_own_cert(&(nettype_tls_params->ssl_conf),
            ...
            HTTP_LOG_E("%s:%d %s()... mbedtls_ssl_conf_own_cert failed returned 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
            RETURN_ERROR(rc);
        }
        ...
        mbedtls_ssl_conf_authmode(&(nettype_tls_params->ssl_conf), MBEDTLS_SSL_VERIFY_NONE);
    }
#endif
    ...
    if ((rc = mbedtls_ssl_setup(&(nettype_tls_params->ssl), &(nettype_tls_params->ssl_conf))) != 0) {
        HTTP_LOG_E("mbedtls_ssl_setup failed returned 0x%04x", (rc < 0 )? -rc : rc);
        RETURN_ERROR(rc);
    }
...
int nettype_tls_connect(network_t* n)
{
    ...
    while ((rc = mbedtls_ssl_handshake(&(nettype_tls_params->ssl))) != 0) {
        if (rc != MBEDTLS_ERR_SSL_WANT_READ && rc != MBEDTLS_ERR_SSL_WANT_WRITE) {
            HTTP_LOG_E("%s:%d %s()...mbedtls handshake failed returned 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
#if defined(MBEDTLS_X509_CRT_PARSE_C)
            if (rc == MBEDTLS_ERR_X509_CERT_VERIFY_FAILED) {
                HTTP_LOG_E("%s:%d %s()...unable to verify the server's certificate", __FILE__, __LINE__, __FUNCTION__);
            }
#endif
            goto exit;
        }
    ...
    if ((rc = mbedtls_ssl_get_verify_result(&(nettype_tls_params->ssl))) != 0) {
        HTTP_LOG_E("%s:%d %s()...mbedtls_ssl_get_verify_result returned 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
        goto exit;
    }

    n->nettype_tls_params = nettype_tls_params;
    n->socket = nettype_tls_params->socket_fd.fd;
    RETURN_ERROR(HTTP_SUCCESS_ERROR)

exit:
    do {
        ret = mbedtls_ssl_close_notify(&(nettype_tls_params->ssl));
    } while (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE);

    mbedtls_ssl_session_reset(&(nettype_tls_params->ssl));
...
int nettype_tls_write(network_t *n, unsigned char *buf, int len, int timeout)
{
    ...
    do {
        ...
        if (rc > 0) {
            ...
            HTTP_LOG_E("%s:%d %s()... mbedtls_ssl_write failed: 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
            break;
        } 
...
int nettype_tls_read(network_t *n, unsigned char *buf, int len, int timeout)
{
    ...
    do {
        ...
        if (rc > 0) {
            ...
            // HTTP_LOG_E("%s:%d %s()... mbedtls_ssl_read failed: 0x%04x", __FILE__, __LINE__, __FUNCTION__, (rc < 0 )? -rc : rc);
            break;
        } 
